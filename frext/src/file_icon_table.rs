//! Generated file-type icon lookup tables for the Catppuccin Mocha
//! icon set (vendored from `catppuccin/zed-icons`, commit
//! `6d953c8c7566b9e721d15b54e802dd77ae26fa9c`).
//!
//! This file is generated. Do not edit by hand; regenerate from the
//! upstream manifest. See `assets/file-icons/` for the SVG assets and
//! the module `crate::file_icon` for the runtime lookup that consumes
//! these tables.

/// The SVG stem used when no suffix or full-name rule matches a file.
pub const DEFAULT_FILE: &str = "_file";

/// The SVG stem for a collapsed directory with no specific rule.
pub const DEFAULT_DIR_COLLAPSED: &str = "_folder";

/// The SVG stem for an expanded directory with no specific rule.
pub const DEFAULT_DIR_EXPANDED: &str = "_folder_open";

/// Lower-priority match: file extension (suffix) to SVG stem.
///
/// Keys are matched case-sensitively against the longest trailing
/// dotted component of the file name, then progressively shorter ones.
pub const FILE_SUFFIXES: &[(&str, &str)] = &[
    ("._sc_", "godot-assets"),
...
/// Highest-priority match: full file name to SVG stem.
pub const FILE_STEMS: &[(&str, &str)] = &[
    (".DS_Store", "macos"),
    ("CMakeLists.txt", "cmake"),
    ("Caddyfile", "caddy"),
    ("Cargo.lock", "cargo-lock"),
...
/// Folder-name (lowercased) to `(collapsed, expanded)` SVG stems.
pub const NAMED_DIRS: &[(&str, &str, &str)] = &[
